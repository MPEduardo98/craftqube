...
// de sobredimensionados) a partir del CP y los ítems del carrito.
// Los pesos/dimensiones son autoritativos: salen de producto_envio.
// ─────────────────────────────────────────────────────────────
import { NextRequest, NextResponse } from "next/server";
import { pool }                       from "@/shared/lib/db/pool";
import type { RowDataPacket }         from "mysql2";
import { calcularEnvio, type EnvioItemRaw, type EnvioCaps, type EnvioTarifa } from "@/features/checkout/lib/cotizarEnvio";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const items: { variante_id: number; cantidad: number }[] = Array.isArray(body.items) ? body.items : [];
    ...
    const varianteIds = items.map((i) => Number(i.variante_id)).filter(Boolean);
    const cantPorVariante = new Map(items.map((i) => [Number(i.variante_id), Math.max(1, Number(i.cantidad) || 1)]));
