...
export function StepConfirmacion({ formData, orderNumber, totalFinal, onClearCart }: Props) {
  useEffect(() => { onClearCart(); }, []); // eslint-disable-line

  const metodoPago = formData.pago.metodo === "tarjeta"
    ? `•••• ${formData.pago.numeroTarjeta.slice(-4)}`
    ...
    >
      ...
      <div className="text-center">
        <motion.h2
          ...
          style={{ fontFamily: "var(--font-display)", fontSize: "clamp(1.6rem,4vw,2rem)", fontWeight: 700, color: "var(--color-cq-text)", letterSpacing: "-0.02em" }}>
          ¡Pedido confirmado!
        </motion.h2>
        <motion.p
          ...
          style={{ fontFamily: "var(--font-body)", fontSize: "0.875rem", color: "var(--color-cq-muted)", marginTop: 8, maxWidth: 360 }}>
          ...
          {details.map(({ label, value, icon }) => (
            <div key={label} className="flex justify-between items-start gap-4">
              <div className="flex items-center gap-2 shrink-0">
                ...
                <span style={{ fontFamily: "var(--font-mono)", fontSize: "0.65rem", letterSpacing: "0.06em", textTransform: "uppercase", color: "var(--color-cq-muted-2)" }}>
                  {label}
                </span>
              </div>
              <span style={{ fontFamily: "var(--font-body)", fontSize: "0.82rem", color: "var(--color-cq-text)", textAlign: "right", fontWeight: label === "Total" ? 700 : 400 }}>
                {value}
              </span>
            </div>
          ))}
      ...
      <motion.div
        ...
        {[
          ...
          <div key={step} className="flex items-start gap-3">
            <i className={icon} style={{ fontSize: "0.8rem", color: "var(--color-cq-accent)", marginTop: 2, flexShrink: 0, width: 16, textAlign: "center" }} />
            <p style={{ fontFamily: "var(--font-body)", fontSize: "0.82rem", color: "var(--color-cq-muted)" }}>{text}</p>
          </div>
        ))}
      </motion.div>
