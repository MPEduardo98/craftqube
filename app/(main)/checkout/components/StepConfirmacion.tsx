...
export function StepConfirmacion({ formData, orderNumber, totalFinal, pedidoId, onClearCart }: Props) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="px-5 py-4" style={{ borderBottom: "1px solid var(--color-cq-border)" }}>
          ...
          <p style={{ fontFamily: "var(--font-body)", fontSize: "0.82rem", color: "var(--color-cq-text)", lineHeight: 1.6 }}>
            ...
            {envio.estado}, {envio.codigoPostal} · México
          </p>
        </div>
        ...
        <div className="px-5 py-4">
          ...
          {pago.metodo === "transferencia" && (
            <p style={{ fontFamily: "var(--font-body)", fontSize: "0.75rem", color: "var(--color-cq-muted)", marginTop: 4 }}>
              Recibirás los datos bancarios en tu correo.
            </p>
          )}
          {pago.metodo === "oxxo" && (
            <p style={{ fontFamily: "var(--font-body)", fontSize: "0.75rem", color: "#ca8a04", marginTop: 4 }}>
              Tienes 72 horas para pagar en cualquier tienda OXXO.
            </p>
          )}
        </div>
      </motion.div>

      {/* ── Nota cuenta invitado ── */}
      {!autenticado && (
        <motion.div
          initial={{ opacity: 0, y: 8 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 0.4 }}
          className="flex flex-col gap-3 py-4 px-5 rounded-xl w-full"
          style={{ background: "rgba(37,99,235,0.05)", border: "1px solid rgba(37,99,235,0.18)", maxWidth: 480 }}
        >
          <div className="flex items-center gap-2">
            <i className="fa-solid fa-user-plus" style={{ fontSize: "0.85rem", color: "var(--color-cq-accent)" }} />
            <p style={{ fontFamily: "var(--font-display)", fontSize: "0.82rem", fontWeight: 700, color: "var(--color-cq-text)" }}>
              Crea tu cuenta y rastrea este pedido
            </p>
          </div>
          <p style={{ fontFamily: "var(--font-body)", fontSize: "0.78rem", color: "var(--color-cq-muted)", lineHeight: 1.55 }}>
            Con una cuenta gratuita puedes ver el estado de este pedido, acceder a tu historial de compras y hacer checkout más rápido la próxima vez.
          </p>
          <Link
            href={`/registro?email=${encodeURIComponent(contacto.email)}`}
            className="flex items-center gap-2 justify-center rounded-xl"
            style={{
              height: 44, background: "var(--color-cq-primary)", color: "white",
      ...
      {/* ── Acceso al pedido para usuarios autenticados ── */}
      ...
      >
        <Link
          ...
          style={{
            height: 48, background: "var(--color-cq-surface)",
            border: "1px solid var(--color-cq-border)",
            color: "var(--color-cq-muted)",
            fontFamily: "var(--font-display)", fontSize: "0.8rem",
            letterSpacing: "0.08em", textTransform: "uppercase",
        ...
        >
          <i className="fa-solid fa-arrow-left" style={{ fontSize: "0.72rem" }} />
          Seguir comprando
        </Link>
        <Link
          href="/"
          className="flex-1 flex items-center justify-center gap-2 rounded-xl"
          style={{
            height: 48, background: "var(--color-cq-primary)", color: "white",
            fontFamily: "var(--font-display)", fontSize: "0.8rem",
            letterSpacing: "0.08em", textTransform: "uppercase",
