...
export async function getProductoBySlug(slug: string): Promise<ProductoDetalle | null> {
  ...
  try {
    ...
    const variantesCompletas = variantes.map((v) => ({
      ...v,
      atributos:  atributosPorVariante[v.id] || [],
      metacampos: metacampos.filter((m) => m.variante_id === v.id),
    }));
    ...
    return {
      ...
      metacampos: metacampos.filter((m) => m.variante_id === null),
      etiquetas,
    } as ProductoDetalle;

  } catch (error) {
    console.error(`[getProductoBySlug] DB error for slug "${slug}":`, error);
    return null;
  } finally {
